	...
	function __construct() {
		...
		if ( !is_admin() ) :
			...
			add_filter( 'wp_redirect', array(&$this, 'redirect'), 1, 2 );
                        add_action( 'deals_before_customer_login_form', array(&$this, 'show_messages'), 10);
			add_action( 'deals_before_single_content', array(&$this, 'show_messages'), 10);
			add_action( 'deals_before_loop', array(&$this, 'show_messages'), 10);
	...
	/*-----------------------------------------------------------------------------------*/ 
		...
		function show_messages() {
			...
			if (isset($this->errors) && sizeof($this->errors)>0) :
				echo '<div class="deals-error">'.$this->errors[0].'</div>';
				$this->clear_messages();
				return true;
			elseif (isset($this->messages) && sizeof($this->messages)>0) :
				echo '<div class="deals-message">'.$this->messages[0].'</div>';
				$this->clear_messages();
				return true;
			else :
				return false;

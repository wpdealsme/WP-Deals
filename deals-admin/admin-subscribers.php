...
require_once DEALS_CLASSES . '/class-deals-list-table.php';
...
function deals_subscriber_bulk_action() {    
    ...
    if($requested_action == 'delete' || $requested_action2 == 'delete') {
        ...
        wp_redirect(admin_url('/admin.php?page=deal-subscribers'));
        exit();        
...
function deals_extra_table_nav($which) {
   ...
   if( ('top' == $which) OR ('bottom' == $which)) {    
        echo '<div class="alignleft actions" style="padding-top: 6px;"><a href="#" style="display: inline;" id="js-import-csv" class="reset-button button-secondary">Import</a> <a href="'.admin_url('/admin.php?page=deal-subscribers&action=export').'" id="export-csv" class="button-primary">'.__('Export', 'wpdeals').'</a></div>';     
   }
...
function deals_export_csv(){    
    ...
    if( isset($_GET['action']) && ($_GET['action'] == 'export') AND ($_GET['post_type'] == 'daily-deals') AND ($_GET['page'] == 'deal-subscribers')){
        ...
        //close file
        fclose($fp);
        wp_redirect(DEALS_FORM_DIR.'subscribers.csv');
        exit();
    }
...
function deals_subscribers() {
    ...
    echo '<form action="'.admin_url('/admin.php?page=deal-subscribers').'" method="post">';    
    ...
    <div id="subscriber-import" style="
        ...
        <div id="js-subscriber-close">
            <a href="#" id="js-close"><?php _e('[close]', 'wpdeals'); ?></a>            
        </div>
        <hr />
        <div id="js-subscriber-content">
            <h3><?php _e('Import CSV', 'wpdeals'); ?></h3>
            <form enctype="multipart/form-data" method="POST" id="subscriber-form" action="">
                ...
                <input type="submit" name="submit" value="<?php _e('Upload', 'wpdeals'); ?>" />
            </form>
        </div>

...
if ( is_admin() ) {
	...
	$deals_page= 'appearance_page_options-framework';
	add_action( "admin_print_styles-$deals_page", 'deals_mlu_css', 0 );
	add_action( "admin_print_scripts-$deals_page", 'deals_mlu_js', 0 );	
}
...
if ( ! function_exists( 'deals_mlu_init' ) ) {
	function deals_mlu_init () {
		register_post_type( 'dealoptions', array(
			'labels' => array(
				'name' => __( 'Deal Options Internal Container' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'deals_mlu_css' ) ) {

	function deals_mlu_css () {
...
if ( ! function_exists( 'deals_mlu_js' ) ) {

	function deals_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'deals-medialibrary-uploader', DEALS_FRAMEWORK_DIRECTORY .'js/deals-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
		wp_enqueue_script( 'deals-medialibrary-uploader' );
 ...
 *
 * Dependencies:
 * - dealsframework_mlu_get_silentpost()
 */

if ( ! function_exists( 'deals_medialibrary_uploader' ) ) {

	function deals_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = deals_mlu_get_silentpost( $id );
		...
		if ( $_desc != '' ) {
			$output .= '<span class="deals_metabox_desc">' . $_desc . '</span>' . "\n";
		}
 ...
 * Return: $_postid.
 *
 * If no "silent" post is present, one will be created with the type "dealsframework"
 * and the post_name of "deals-$_token".
 *
 * Example Usage:
 * dealsframework_mlu_get_silentpost ( 'deals_logo' );
 */

if ( ! function_exists( 'deals_mlu_get_silentpost' ) ) {

	function deals_mlu_get_silentpost ( $_token ) {
	
		global $wpdb;
		$_id = 0;
	
		// Check if the token is valid against a whitelist.
		// $_whitelist = array( 'deals_logo', 'deals_custom_favicon', 'deals_ad_top_image' );
		// Sanitise the token.
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'dealoptions', 'post_name' => 'deals-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'deals_mlu_insidepopup' ) ) {

	function deals_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_dealoptions'] ) && $_REQUEST['is_dealoptions'] == 'yes' ) {
		
			add_action( 'admin_head', 'deals_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'deals_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'deals_mlu_js_popup' ) ) {

	function deals_mlu_js_popup () {

		$_deals_title = $_REQUEST['deals_title'];
		if ( ! $_deals_title ) { $_deals_title = 'file'; } // End IF Statement
?>
	...
	jQuery(function($) {
		...
		$( 'h3.media-title' ).each ( function () {
			...
			var new_title = current_title.replace( 'media file', '<?php echo $_deals_title; ?>' );
			$( this ).html( new_title );
		...
		// Preserve the "is_dealsframework" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_dealsframework=yes';
...
if ( ! function_exists( 'deals_mlu_modify_tabs' ) ) {

	function dealsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'wpdeals' ), __( 'Previously Uploaded', 'wpdeals' ), $tabs['gallery'] );
		return $tabs;
	}
}
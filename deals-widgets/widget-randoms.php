...
class Deals_Random_Widget extends WP_Widget {
	 ...
	 * @return void
	 **/
	function Deals_Random_Widget() {
		$widget_ops = array( 'classname' => 'widget_deals_randoms', 'description' => __( 'Use this widget to list your random deals', 'wpdeals' ) );
		$this->WP_Widget( 'widget_deals_randoms', __( 'Deals - Randoms', 'wpdeals' ), $widget_ops );
		$this->alt_option_name = 'widget_deals_randoms';
	...
	function widget( $args, $instance ) {
		$cache = wp_cache_get( 'widget_deals_randoms', 'widget' );
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Randoms Deals', 'wpdeals' ) : $instance['title'], $instance, $this->id_base);
			...
			<li class="widget-deal-item">
                            <a href="<?php the_permalink(); ?>">
                                <?php if (has_post_thumbnail()) : ?>
                                        <span class="random-image-wrap image-wrap">                                                
                                                <?php deals_image( 'width='.$width.'&height='.$height.'&class=deal-thumbnail'); ?>
                                        </span>
                                <?php endif; ?>

                                <div class="random-deal-title">
                                        <?php the_title(); ?>
                                </div>
                            </a>
			</li>
		...
		// end check for randomsl posts
		endif;

		$cache[$args['widget_id']] = ob_get_flush();
		wp_cache_set( 'widget_deals_randoms', $cache, 'widget' );
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['widget_deals_randoms'] ) )
			delete_option( 'widget_deals_randoms' );

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete( 'widget_deals_randoms', 'widget' );
	}
